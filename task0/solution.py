...
def log_posterior_probs(x):
    ...
    log_likelihood = np.array([dist.logpdf(x).sum() for dist in HYPOTHESIS_SPACE])
    unnormalized_log_posterior = np.log(PRIOR_PROBS) + log_likelihood
    normalizer = logsumexp(unnormalized_log_posterior)
    log_p = unnormalized_log_posterior - normalizer
