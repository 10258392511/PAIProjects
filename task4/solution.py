...
###
def to_numpy(tensor: Union[torch.Tensor, np.ndarray]):
    tensor_out = tensor
    if isinstance(tensor, torch.Tensor):
        dtype = tensor_out.dtype
        tensor_out = tensor_out.detach().cpu().to(dtype).numpy()
...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    sizes: List
        List of feature sizes, i.e.,
            [input_dim, hidden_layer_1, ..., hidden_layer_n_dim, output_dim]
...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ### Note: act_dim is actually number of actions, since each action is a number
        super().__init__()
    ...
    def _distribution(self, obs):
        ...
        # distribution.
        act_logits = self.logits_net(obs.float())  # (n, num_act)
        distr = torch.distributions.Categorical(logits=act_logits)
...
class VPGBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma, lam):
        ...
        self.tdres_buf = np.zeros(size, dtype=np.float32)  ### also: advantages
        # rewards
    ...
    def store(self, obs, act, rew, val, logp):
        ...
        self.obs_buf[self.ptr] = to_numpy(obs)
        self.act_buf[self.ptr] = to_numpy(act)
        self.rew_buf[self.ptr] = to_numpy(rew)
        self.val_buf[self.ptr] = to_numpy(val)
        self.logp_buf[self.ptr] = to_numpy(logp)
    ...
    def end_traj(self, last_val=0):
        ...
        self.tdres_buf[path_slice] = discount_cumsum(rews[:-1] + self.gamma * vals[1:] - vals[:-1],
                                                     self.gamma * self.lam)
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            act = pi.sample().squeeze()  # (1,) -> float
            v = self.critic(state.float())  # (1,), already squeezed
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        _, logp_actor = agent.actor(data["obs"], act=data["act"])
        actor_loss = -(logp_actor * data["tdres"]).sum()
        actor_loss.backward()
        ...
        for _ in range(100):
            ...
            v_vals = agent.critic(data["obs"].float())
            critic_loss = torch.mean((v_vals - data["ret"]) ** 2)
