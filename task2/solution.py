...
# new lines added are commented with ###
...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        self.layer1 = nn.Linear(in_features, 1000)
        self.layer2 = nn.Linear(1000, 100)
        # self.layer21 = nn.Linear(500, 100)
    ...
    def forward(self, x):
        ...
        # x = F.dropout(
        #     F.relu(self.layer21(x)),
        #     p=self.dropout_p,
        #     training=self.training or self.dropout_at_eval
        # )

        class_probs = self.layer3(x) / self.T
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
                 *args, **kwargs):
        """
        kwargs: weight_decay (new), dropout (new)
        """
        super().__init__(dataset_train, *args, **kwargs)
        ...
        self.num_epochs = 100
        self.dropout = kwargs.get("dropout", 0.6)  ### from the paper
        self.weight_decay = kwargs.get("weight_decay", 0.001)  ### new hyper-param
        ...
        self.network = MNISTNet(in_features= 28 * 28, out_features=10, dropout_p=self.dropout, dropout_at_eval=True)
        # self.network = SelfMadeNetwork(in_features= 1, out_features=10, dropout_p=self.dropout, dropout_at_eval=True)
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate,
                                          weight_decay=self.weight_decay)

        ### our implementations
        self.loss_fn = nn.CrossEntropyLoss()  # L2 regularization goes into the optimizer
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                y_pred = self.network(batch_x)  # (B, num_cls)
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        assert x.shape[1] == 28 ** 2
        self.network.eval()  ### note dropout is implemented by F.dropout(.), so this doesn't turn dropout off
        ...
        estimated_probability = torch.zeros(x.shape[0], 10, device=x.device, dtype=x.dtype)
...
class BackpropTrainer(Framework):
    def __init__(self, dataset_train,  *args, **kwargs):
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.num_epochs, eta_min=1e-5)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                pred, log_prior, log_var_posterior = self.network(batch_x)  # (B, C), (1,), (1,)
                kl_div = (log_var_posterior - log_prior) / num_batches
                loss = self.loss_fn(pred, batch_y) + kl_div
                # self.optimizer.zero_grad()
                loss.backward()
                # nn.utils.clip_grad_value_(self.network.parameters(), 0.01)
                self.optimizer.step()
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.use_bias = bias
        fan_in_out_norm = np.sqrt(2 / (in_features + out_features))
        ...
        self.prior = UnivariateGaussian(torch.tensor(0.), torch.tensor(fan_in_out_norm))
        ...
        #         nn.Parameter(torch.zeros(out_features, in_features).uniform_(-fan_in_out_norm, fan_in_out_norm)),
        #         nn.Parameter(torch.zeros(out_features, in_features).uniform_(-fan_in_out_norm, fan_in_out_norm))
        # )
        self.weights_var_posterior = MultivariateDiagonalGaussian(
            nn.Parameter(torch.zeros(out_features, in_features)),
            nn.Parameter(torch.ones(out_features, in_features) * fan_in_out_norm)
        )
        ...
        if self.use_bias:
            ...
            #     nn.Parameter(torch.zeros(out_features).uniform_(-fan_in_out_norm, fan_in_out_norm)),
            #     nn.Parameter(torch.zeros(out_features).uniform_(-fan_in_out_norm, fan_in_out_norm))
            # )
            self.bias_var_posterior = MultivariateDiagonalGaussian(
                nn.Parameter(torch.zeros(out_features)),
                nn.Parameter(torch.ones(out_features) * fan_in_out_norm)
            )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        weights = self.weights_var_posterior.sample()  # (out_features in_features)
        bias = None
        if self.use_bias:
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for i, layer_iter in enumerate(self.layers):
            x, log_prior_iter, log_variational_posterior_iter = layer_iter(x)
            log_prior += log_prior_iter
            log_variational_posterior += log_variational_posterior_iter
            if i < len(self.layers) - 1:
                x = self.activation(x)

        output_features = x / self.T
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        log_lh = -0.5 * np.log(2 * np.pi) - torch.log(self.sigma) - ((values - self.mu) ** 2) / (2 * self.sigma ** 2)
        log_lh = log_lh.sum()
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        # print(f"mu: {self.mu.flatten()[0]}, rho: {self.rho.flatten()[0]}")
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        log_lh = -0.5 * np.log(2 * np.pi) - torch.log(sigma) - \
                 ((values - self.mu) ** 2) / (2 * sigma ** 2)
        log_lh = log_lh.sum()
